...
const UserLogin: React.FC<UserLoginProps> = ({ onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
  const res = await fetch('http://192.168.1.14:4000/login', {
        method: 'POST',
      ...
      <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-2xl" style={{ boxShadow: '0 8px 32px rgba(244,163,0,0.15)' }}>
        <div className="flex flex-col items-center justify-center mb-8">
          <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.15em', marginBottom: '0.2em' }}>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#0055aa', letterSpacing: '2px' }}>C</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#009933', letterSpacing: '2px' }}>A</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#222', letterSpacing: '2px' }}>T</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#222', letterSpacing: '2px' }}>A</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#009933', letterSpacing: '2px' }}>L</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#0055aa', letterSpacing: '2px' }}>Y</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#994400', letterSpacing: '2px' }}>S</span>
              <span style={{ fontSize: '2.8em', fontWeight: 900, color: '#994400', letterSpacing: '2px' }}>T</span>
            </div>
            <div style={{
              fontSize: '1.1em',
              fontWeight: 700,
