...
const API_URL = 'http://192.168.1.14:4000/sites'; // Updated to your backend IP
...
const AdminLocations: React.FC = () => {
  ...
  const handleAddCanteenForSite = async (siteLocation: string) => {
    ...
    try {
  const res = await fetch('http://192.168.1.14:4000/admin/canteens', {
        method: 'POST',
  ...
  const res2 = await fetch(`http://192.168.1.14:4000/canteens?site=${encodeURIComponent(siteLocation)}`);
  ...
  const handleDeleteCanteenForSite = async (siteLocation: string, canteenName: string) => {
    try {
  const res = await fetch(`http://192.168.1.14:4000/admin/canteens?site=${encodeURIComponent(siteLocation)}&name=${encodeURIComponent(canteenName)}`, {
        method: 'DELETE'
      });
      const data = await res.json();
  ...
  useEffect(() => {
    async function fetchCanteens() {
      ...
      for (const site of sites) {
        try {
          const res = await fetch(`http://192.168.1.14:4000/canteens?site=${encodeURIComponent(site.location)}`);
          const data = await res.json();
              ...
              <React.Fragment key={(site.id ?? '') + site.location + site.branch_location}>
                <tr className="border-b">
                  ...
                  <td className="p-2 flex gap-2">
                    <button onClick={() => {/* edit logic here */}} className="bg-blue-500 text-white px-2 py-0.5 rounded-md mr-1 text-xs">Edit</button>
                    <button onClick={() => {/* delete logic here */}} className="bg-red-500 text-white px-2 py-0.5 rounded-md text-xs">Delete</button>
                    <button
          ...
          <div className="bg-white rounded shadow-lg p-6 min-w-[320px] max-w-[90vw]">
            ...
            <form
              onSubmit={async e => {
                ...
                try {
                  const res = await fetch(`${API_URL}/${editSiteId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
