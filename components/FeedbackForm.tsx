...
const FeedbackForm: React.FC<FeedbackFormProps> = ({ jwt, site: propSite }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
  fetch(`http://192.168.1.14:4000/canteens?site=${encodeURIComponent(site)}`).then(res => res.json()).catch(() => null),
  fetch(`http://192.168.1.14:4000/questions?site=${encodeURIComponent(site)}`).then(res => res.json()).catch(() => null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
  const res = await fetch('http://192.168.1.14:4000/feedback', {
        method: 'POST',
        headers: {
  ...
  return (
  <div className="card" style={{ maxWidth: 320, margin: '1em auto', padding: '1em' }}>
    <div className="text-center mb-2">
      <h1 className="font-bold text-base md:text-xl text-gray-800 mb-1">Canteen Feedback System</h1>
      <h2 className="text-base font-bold text-gray-800">Share Your Feedback</h2>
      <p className="text-gray-500 mt-1 text-xs">Help us improve your dining experience!</p>
      ...
      {/* Removed Manage Questions link from feedback page */}
    </div>
       <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <button
          ...
          className="w-full bg-primary-600 text-white font-bold py-1 px-2 rounded hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-300 transition-all duration-300 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-xs h-8"
        >
          {isSubmitting ? (
